...
Range Pools are similar to what users expect from modern-day AMMs while setting liquidity between a specified price range (UniV3 Concentrated Liquidity). 

Liquidity providers can provide their liquidity to a specific range, resulting in a higher concentration of liquidity and less slippage for swappers compared to AMM without price bounds (UniV2 Infinite Price Bound Range) due to the liquidity setting to a specific price range rather than providing the Full Range of a constant product curve. 

The opportunity cost of providing liquidity to a constant function curve is known as impermanent loss. Cover Pools are designed to be used to hedge against impermanent loss.
Range Pools have one unique feature that improves the composability with other DeFi protocols: Default Ranges

graphic 1

Range Pools contain many small content function curves between each price point, commonly called a Tick. 
...
Claiming Fees Accrued 
How Range Pools track how much the users' positions have been filled is by using the value `feeGrowthGLobal`. Depending on where the current tick is it will determine what liquidity is withdrawn from the pool. 
